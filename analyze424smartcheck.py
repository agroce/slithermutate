...
CONTRACTS = CONTRACTS[:60] # just first 60 for first run

already_done = []
if os.path.exists("424.analyzed.smartcheck.txt"):
    with open("424.analyzed.smartcheck.txt", 'r') as finished:
...
for C in CONTRACTS:
    ...
    if C in already_done:
        ...
        with open(C+".smartcheck.issues", 'r') as issuef:
            numIssues = int(issuef.read())
        print("ISSUES:", numIssues)
        if numIssues == -1:
            print("SMARTCHECK FAILED")
            continue
        if os.path.exists(C+".smartcheck.killed.txt"):
            killedCount = 0
            notKilledCount = 0
            with open(C+".smartcheck.killed.txt", 'r') as killed:
                for line in killed:
                    killedCount += 1
            with open(C+".smartcheck.notkilled.txt", 'r') as notKilled:
        ...
        with open("out.txt", 'w') as outf:
            numIssues = runSmartcheck(C)
        with open("out.txt", 'r') as outf:
            results = outf.read()
        print("ISSUES:", numIssues)
        with open(C+".smartcheck.issues", 'w') as issuef:
        ...
        if numMutants == 0:
            ...
            continue
        with open("out.txt", 'w') as outf:
            subprocess.call(["analyze_mutants", C, "python maxissuessmartcheck.py " +
                                 str(numIssues) + " " + C,
        ...
        sys.stdout.flush()    
        subprocess.call(["cp","killed.txt",C+".smartcheck.killed.txt"])
        subprocess.call(["cp","notkilled.txt",C+".smartcheck.notkilled.txt"])
        with open("424.analyzed.smartcheck.txt", 'a') as finished:
            finished.write(C + "\n")
    scores.append(score)
