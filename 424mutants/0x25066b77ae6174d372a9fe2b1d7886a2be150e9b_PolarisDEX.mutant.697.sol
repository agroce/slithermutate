...
contract PolarisDEX is SafeMath {
  ...
  function testTrade(address tokenGet, uint amountGet, address tokenGive, uint amountGive, uint expires, uint nonce, address user, uint8 v, bytes32 r, bytes1 s, uint amount, address sender) constant public returns(bool) {
    if (!(
