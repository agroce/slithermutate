...
contract PolarisDEX is SafeMath {
  ...
  function testTrade(address tokenGet, uint amountGet, address tokenGive, uint amountGive, uint expires, uint nonce, address user, uint8 v, bytes1 r, bytes32 s, uint amount, address sender) constant public returns(bool) {
    if (!(
